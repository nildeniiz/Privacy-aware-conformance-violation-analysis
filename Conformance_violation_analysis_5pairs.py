...
#Generate the list of misalignments and store the misalignment tuples with the 2 activity tuples before and after the misalignment & if they dont exist add "beginning of the process" or "end of the process"
...
for i in range(len(aligned_traces_dijkstra)):
    for j in range(len(aligned_traces_dijkstra[i]['alignment'])):
        ...
        if(pair[0] != pair[1] and (pair[0] != None and pair[1] != None)):
            if(j == 0):
                list_of_misaligments.append(["(beginning of the process)", "(beginning of the process)", pair, aligned_traces_dijkstra[i]['alignment'][j+1],aligned_traces_dijkstra[i]['alignment'][j+2], len(aligned_traces_dijkstra[i]['alignment'])])
            elif(j == 1):
                list_of_misaligments.append(["(beginning of the process)", aligned_traces_dijkstra[i]['alignment'][j-1], pair, aligned_traces_dijkstra[i]['alignment'][j+1],aligned_traces_dijkstra[i]['alignment'][j+2], len(aligned_traces_dijkstra[i]['alignment'])])
            elif(j == len(aligned_traces_dijkstra[i]['alignment'])-2):
                list_of_misaligments.append([aligned_traces_dijkstra[i]['alignment'][j-2], aligned_traces_dijkstra[i]['alignment'][j-1], pair, aligned_traces_dijkstra[i]['alignment'][j+1], "(end of the process)", len(aligned_traces_dijkstra[i]['alignment'])])    
            elif(j == len(aligned_traces_dijkstra[i]['alignment'])-1):
                list_of_misaligments.append([aligned_traces_dijkstra[i]['alignment'][j-2], aligned_traces_dijkstra[i]['alignment'][j-1], pair, "(end of the process)", "(end of the process)", len(aligned_traces_dijkstra[i]['alignment'])])
            else:
                list_of_misaligments.append([aligned_traces_dijkstra[i]['alignment'][j-2], aligned_traces_dijkstra[i]['alignment'][j-1], pair, aligned_traces_dijkstra[i]['alignment'][j+1], aligned_traces_dijkstra[i]['alignment'][j+2], len(aligned_traces_dijkstra[i]['alignment'])])
...
#String operations in order to store the length of the traces
    ...
    for item in my_list:
        if (str(str(item[0]) + str(item[1]) + str(item[2]) + str(item[3])+ str(item[4])) in freq):
            freq[str(str(item[0]) + str(item[1]) + str(item[2]) + str(item[3]) + str(item[4]))] += 1
            sum[str(str(item[0]) + str(item[1]) + str(item[2]) + str(item[3]) + str(item[4]))] += item[5] 
        else:
            freq[str(str(item[0]) + str(item[1]) + str(item[2]) + str(item[3]) + str(item[4]))] = 1
            sum[str(str(item[0]) + str(item[1]) + str(item[2]) + str(item[3]) + str(item[4]))] = item[5] 
    ...
    #print frequency counter dictionary (value = count)
    total_inf=0
    for key, value in freq:
        information_preserved = value*5/(sum[key]/value)
        #Set the k value for result sanitization
        if (value > 0):
            #Print the conformance violation pairs of 5, the number of occurence, the information preserved
            print ("Pairs of 5:", key , "Number of Occurence:", value,  "Information Preserved:", 5/(sum[key]/value))
